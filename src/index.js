...
const Graph = ({
  ...
  const handleZoomIncrease = () => setK(k + 0.01);
  const handleZoomDecrease = () => setK(k - 0.01);

  return (
    <>
      <button onClick={handleZoomIncrease}>Increase</button>
      <button onClick={handleZoomDecrease}>Decrease</button>
      <svg id={id} width="100%" height="100%" {...restProps} ref={svgRef}>
        <g
